...
function calculatePhysics() {
  ...
  if (!category || !formulaKey) {
    resultDiv.innerHTML = '<div class="alert alert-warning py-2 px-3">Please select both category and formula</div>';
    return;
  }
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    if (input && input.style.display !== 'none') {
      ...
      if (isNaN(value)) {
        resultDiv.innerHTML = '<div class="alert alert-danger py-2 px-3">Please enter valid numbers for all inputs</div>';
        return;
      }
      inputs.push(value);
    }
  ...
  try {
    ...
    if (isNaN(result) || !isFinite(result)) {
      resultDiv.innerHTML = '<div class="alert alert-danger py-2 px-3">Error in calculation. Please check your inputs.</div>';
      return;
    }
    ...
    resultDiv.innerHTML = '<div class="alert alert-danger py-2 px-3">Error in calculation: ' + error.message + '</div>';
  }
}
...
function clearPhysicsCalculator() {
  ...
  document.getElementById('physics-formula').innerHTML = '<option value="">-- Select Formula --</option>';
